...
And we can create variables using ```html var @variableName``` and define a value using ```html set @variableName = "Value"```
...
E nós podemos criar variáveis usando ```html var @nomeVariavel``` e definir valores usando ```set @nomeVariavel = "valor"```
